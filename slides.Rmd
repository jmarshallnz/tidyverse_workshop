...
You are recommended to use the [RStudio IDE](https://www.rstudio.com/products/rstudio/), but you don't have to.
...
Number of school kids by school, gender, ethnic group and level for 2007 through 2017.

Each row represents a school, gender, ethnic group, level and year.
```{r}
roll
```
...
---
class: inverse
background-image: url('libs/images/opinion.jpg')
...
knitr::include_graphics('images/argh.png')
```
...
knitr::include_graphics('https://media.giphy.com/media/Ls6ahtmYHU760/giphy.gif')
```
...
.pull-right[
```{r, echo=FALSE, out.width=480}
knitr::include_graphics('https://media.giphy.com/media/tCV2LrPPYfqCY/giphy.gif')
```
]
...
This makes it easy to chain functions together:
...
```{r}
roll %>% filter(rank(desc(Students)) <= 3)
```
...
```{r}
roll %>% select(SchoolName, EthnicGroup, Students)
```
...
```{r}
roll %>% mutate(GendEth = paste(EthnicGroup, Gender)) %>%
  select(-Gender, -EthnicGroup)
```
...
```{r}
clean %>% filter(Year == 2017, SchoolName == "Awatapu College",
                 Level == 13) %>%
...
```{r}
clean %>% group_by(SchoolName) %>% head
clean %>% group_by(SchoolName) %>% class
```
...
```{r}
clean %>%
  filter(Year == 2017, EthnicGroup == "Pasifika") %>%
  group_by(SchoolName, Gender) %>%
  summarise(Students = sum(Students)) %>%
