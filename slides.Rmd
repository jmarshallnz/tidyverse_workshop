...
Grab the markdown file:

http://shiny.massey.ac.nz/jcmarsha/tidyverse.Rmd

Run the first chunk to load the libraries/data.

```{r, eval=FALSE, tidy=FALSE}
library(tidyverse)
schools=readRDS('http://shiny.massey.ac.nz/jcmarsha/tidy/schools.rds')
roll=readRDS('http://shiny.massey.ac.nz/jcmarsha/tidy/roll.rds')
```
...
```{r}
clean %>%
  filter(SchoolName == "Awatapu College", Level == 13) %>%
...
clean %>% group_by(SchoolName) %>%
  summarise(Students = sum(Students)) %>%
  left_join(schools, by='SchoolName') %>%
  select(SchoolName, Students, TerritorialAuthority)
```
--
.topright[
